...
public static class ManualPdfGenerator
{
    ...
    private static void AddMainWindow(ColumnDescriptor column)
    {
        ...
        Numbered(column, 3, "Создание полного или сводного PDF на вкладке «Отчёт».");
    ...
    private static void AddReportTab(ColumnDescriptor column)
    {
        ...
        Paragraph(column, "Кнопки PDF активны только после успешного полного сканирования.");
        Paragraph(column,
            "PDF используют шрифты с поддержкой кириллицы и нормализацию текста. " +
            "В отчёт входят признаки очистки с колонкой «Тип действия» (запуск утилиты / вероятная очистка / очистка журналов) " +
            "и, при наличии, раздел «Сторонние утилиты».");
    }
