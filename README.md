...
| Сети (реестр и файлы) | `NetworkList\Profiles` и подписи, `Tcpip\Parameters\Interfaces`, профили Wlansvc, `BTHPORT`/`BTHENUM`, `HKCU\Network`, MRU, `Terminal Server Client` |
| Сети (журналы) | WLAN-AutoConfig, NetworkProfile, SMBClient (Connectivity/Operational/Security), TerminalServices (клиент и сервер), RasClient |
| Браузеры | История и загрузки Chromium-браузеров (Edge, Chrome, Yandex, Brave, Vivaldi) и Firefox |
...
- Live-мониторинг USB/Type-C по **событиям PnP Windows** (без постоянного опроса каждые 2 секунды). При подключении устройства, которого нет в доказательной базе прошлых сканирований, поднимается **алерт «Неизвестное устройство»**: поверх остальных окон открывается окно с накопительным списком таких устройств, каждое дублируется в журнал GUI. Сопоставление идёт по серийному номеру, VID/PID и DeviceInstanceId; повторные срабатывания по одному и тому же устройству в рамках сеанса мониторинга подавляются.
...
3. Изучить вкладки **USB устройства**, **Сетевые подключения**, **Доказательства**, **Следы очистки**.
...
# Список сохранённых сессий (без сканирования, без прав администратора)
UsbForensicAudit.Cli.exe --list-sessions

# Сравнение двух сессий: что появилось и что исчезло между сканированиями
UsbForensicAudit.Cli.exe --diff <id-старой-сессии> <id-новой-сессии> --json diff.json

# Офлайн-анализ чужой системы: смонтированный образ диска или копия каталога Windows
UsbForensicAudit.Cli.exe --offline F:\ --json offline.json --reports .\reports

# Верификация целостности доказательной базы (без сканирования)
UsbForensicAudit.Cli.exe --verify --json integrity.json

# Английский язык сообщений (по умолчанию берётся язык системы)
UsbForensicAudit.Cli.exe --lang en --list-sessions
```

Сравнение сопоставляет устройства по каноническому идентификатору, а
доказательства — по содержательным полям, поэтому результат устойчив к
порядку обнаружения. Исчезнувшие доказательства выделяются отдельно: артефакты
не пропадают сами по себе, их исчезновение — признак очистки следов или
ротации журналов.

Офлайн-режим (`--offline`) анализирует кусты реестра чужой системы:
`SYSTEM` (USBSTOR, USB, MountedDevices), `SOFTWARE` (Windows Portable
Devices, дата установки ОС) и `NTUSER.DAT` профилей из `Users`
(MountPoints2 — какой пользователь обращался к какому тому), а также
файловые артефакты профилей: LNK-ярлыки каталога Recent и рабочего стола
плюс Jump Lists — какие файлы открывали со съёмного носителя и кто именно.
Кусты предварительно копируются во временный каталог, файловые артефакты
только читаются — исследуемые файлы не изменяются. Журналы событий, WMI и работающие процессы чужой машины
недоступны по определению, о чём отчёт честно предупреждает. Результат
сохраняется в ту же базу сессий с префиксом `offline-` и доступен для
`--diff` и генерации отчётов.

Каждая сессия «запечатывается»: журнал доказательств `evidence.jsonl`
ведётся как hash-chain (каждая запись ссылается на SHA-256-хеш предыдущей),
а итоговый хеш сессии сохраняется печатью в `audit.sqlite`. Команда
`--verify` пересчитывает всю цепочку и сверяет печати: любая правка,
удаление или вставка записи задним числом обнаруживается с точностью до
строки журнала. Сессии, записанные до внедрения печатей, помечаются как
«без печати», но нарушением не считаются.

Сообщения CLI локализованы: русский — базовый язык, английский доступен
через `--lang en` или наследуется от языка системы. Строки лежат в
`Resources/Strings.resx` (ru) и `Resources/Strings.en.resx` (en);
отсутствующий в переводе ключ прозрачно откатывается на русский.

Коды возврата: `0` — успех, `1` — ошибка, `2` — нет прав администратора,
`3` — прервано (Ctrl+C), `4` — верификация нашла нарушения целостности,
`64` — неверные аргументы. Справка: `--help`.
