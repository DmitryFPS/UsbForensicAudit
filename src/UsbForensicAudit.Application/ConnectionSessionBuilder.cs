...
public static class ConnectionSessionBuilder
{
        ...
        IEnumerable<(DateTimeOffset TimestampUtc, bool IsConnect, string Provenance)> events)
    {
        ...
        foreach (var item in ordered)
        {
            ...
            if (open is null)
            {
                // Отключение без парного подключения: журнал начинается позже,
                // чем устройство подключили (подключение вытеснено из лога по
                // кругу). Раньше такое событие выбрасывалось целиком — терялся
                // сам факт, что ДО этого момента устройство было в машине.
                // Теперь сеанс сохраняется с пометкой «начало неизвестно».
                sessions.Add(new ConnectionSession
                {
                    StartUtc = item.TimestampUtc,
                    IsStartUnknown = true,
                    EndUtc = item.TimestampUtc,
                    EndProvenance = item.Provenance
                });
                continue;
            }
