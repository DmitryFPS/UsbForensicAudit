...
public static class ManualPdfGenerator
{
    ...
    private static void AddDevicesTab(ColumnDescriptor column)
    {
        ...
        SubTitle(column, "Одно устройство — одна строка");
        Paragraph(column,
            "Windows описывает одно устройство несколькими записями. У телефона по Bluetooth это полтора " +
            "десятка услуг, у встроенной камеры — видеопоток и микрофон, у флешки — записи шины, накопителя " +
            "и переносимого устройства. В списке устройство стоит одной строкой, остальные его записи " +
            "свёрнуты в неё.");
        Bullet(column, "Свёрнутые записи видны по двойному щелчку по строке — в окне сведений, "
                       + "в разделе «Из чего состоит эта строка списка».");
        Bullet(column, "Галочка «Показывать все записи реестра» возвращает их в таблицу: счёт строк тогда "
                       + "сходится с числом записей в реестре, и отчёт можно проверить.");
        Bullet(column, "Счётчик над таблицей показывает, сколько строк видно из скольких найденных записей.");
        Bullet(column, "Части самой машины — корневые концентраторы, контроллеры, служебные узлы шины — "
                       + "тоже свёрнуты: они ничего не говорят о принесённых устройствах.");
        Bullet(column, "Имя вещи важнее имени класса. Windows зовёт запись «USB Composite Device», а модель "
                       + "пишет у её функции — «Integrated Camera»; в списке стоит имя вещи, и в досье отчёта "
                       + "записано, у какой записи оно взято.");

        SubTitle(column, "Цвет строк");
        Paragraph(column,
            "Цвет отвечает на один вопрос: принесли устройство с собой или оно всегда было внутри машины.");
        AddTable(column,
            ("Цвет", "Что это"),
            ("Зелёный", "Внешний носитель или телефон — то, чем выносят данные"),
            ("Синий", "Другое внешнее устройство: пара по Bluetooth, карта памяти, устройство по Type-C или USB4"),
            ("Жёлтый", "На шине USB, но отличить внешнее от распаянного внутри по следам нельзя"),
            ("Серо-синий", "Встроенное в машину, а также части устройств и шины: корневые концентраторы, "
                           + "контроллеры, интерфейсы составных устройств, услуги сопряжённых устройств Bluetooth"),
            ("Фиолетовый", "Виртуальное устройство, созданное программой"),
            ("Сливовый", "След в реестре, самого устройства уже нет"),
            ("Коричневый", "Запись, которую разобрать не удалось"));

        SubTitle(column, "Столбцы таблицы");
        AddTable(column,
            ("Столбец", "Что означает"),
            ("Внешнее устройство?", "Приносили ли устройство с собой — тот же признак, что и цвет строки"),
            ("Что это за запись", "Краткое описание простым языком"),
            ("Имя устройства", "Как Windows называет устройство. Имена, которые Windows хранит ссылкой на строку "
                                + "внутри файла драйвера, разбираются: в таблице стоит имя, а не ссылка"),
        ...
        SubTitle(column, "Устройства Bluetooth");
        Bullet(column, "Само сопряжённое устройство — телефон, гарнитура, часы — стоит в списке как внешнее.");
        Bullet(column, "Windows заводит запись на каждую его услугу: громкая связь, передача файлов, "
                       + "доступ к книге контактов, доступ к сообщениям, выход в сеть. У одного телефона таких "
                       + "записей полтора десятка, и все они свёрнуты в строку самого телефона.");
        Bullet(column, "Перечень услуг важен сам по себе: по нему видно, что через сопряжение было можно. "
                       + "Он открывается двойным щелчком по телефону и печатается в его досье в отчёте.");
        Bullet(column, "Что через сопряжение было можно и когда его использовали — во вкладке «Сетевые подключения».");

        SubTitle(column, "Обстановка сейчас");
        Bullet(column, "Вкладка «Обстановка сейчас» внутри «Сетевые подключения»: снимок Wi-Fi в эфире и устройств в текущей сети.");
        Bullet(column, "Кнопка «Снять обстановку» — после полного сканирования. Галочка «Опросить сеть активно» медленнее, но находит больше устройств.");
        Bullet(column, "Список устройств в сети никогда не полный: Windows не хранит всех клиентов Wi-Fi, видны только соседи из таблицы ARP и ответившие на опрос.");
        ...
        Paragraph(column,
            ...
            "Внизу — раздел «Из чего состоит эта строка списка» с перечнем свёрнутых записей и кнопка «История действий на устройстве».");
    }
    ...
    private static void AddReportTab(ColumnDescriptor column)
    {
        ...
        SubTitle(column, "Устройства в отчёте");
        Bullet(column, "Досье пишется на устройство, а не на запись реестра: записи о его частях перечислены "
                       + "внутри досье, в разделе «Записи Windows об этом устройстве».");
        Bullet(column, "Таблица устройств содержит все записи целиком. Колонка «Место в списке устройств» "
                       + "показывает, свёрнута запись в своё устройство или стоит отдельной строкой.");
        Paragraph(column,
